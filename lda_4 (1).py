...
'whose','why','will','with','without','wont','would','wouldnt','bobsnewline','i','u','bobsnewlin','p','b','x','o','xd','lol'] 
...
#			stemmed_tokens = [p_stemmer.stem(i) for i in stopped_tokens]
   ...
   # 	new_dict.filter_extremes(no_below=3)
    #	new_dict.save('filtered.dict')

   # now transform the corpus
 #   	corpus = corpora.MmCorpus('corpus')
     #   corpus = [new_dict.doc2bow(t) for t in dictionary_words]
	...
	numTopics= 5
	ldamodel = gensim.models.ldamodel.LdaModel(corpora.MmCorpus.serialize, id2word = dictionary, num_topics=numTopics,  passes=20)
